...
public boolean b_Configure = true, b_Shooting = false, b_Hit_detected = false, b_show_last_hit, b_sound, b_ballistics, b_shortcuts = true, b_shoot_stop = false, b_shooting_prepare = false, b_time_of_shooting_s = false; // Mode
public boolean b_training_style = false, b_hunting_style = false, b_combat_style = false, b_sport_style = false; //Shooting style
public boolean b_autoshootlog = true, b_autoshootlog_written = false;
...
public int i_delay, i_shooting_prepare, i_time_of_shooting_s = 0;;
...
{
  if(b_sport_style || b_hunting_style)
  {
    f_shooting_time--;
  ...
  // for shotting style except training
  if((b_time_of_shooting_s == true))
   {
    i_time_of_shooting_s++;
   //System.out.println("i_time_of_shooting_s: " + i_time_of_shooting_s);   
   }
}
...
{
  dia = projectile_diameter * f_diameter_scale;
  if((f_hit_X > 15) && (f_hit_X < video_width -15))
    {
      if((f_hit_Y > 0) && (f_hit_Y < video_height))
      {
        //color of hit
        ...
        //dia = projectile_diameter * f_diameter_scale;
        ellipse(f_hit_X + video_X_offset, f_hit_Y + video_Y_offset, dia, dia);
        }
      }
    };
};
...
public void stop_analyze_when_hit_detected()
{
  if(b_Shooting)
    {
      if((f_hit_X > 15) && (f_hit_X < 625))
      {
        if((f_hit_Y > 15) && (f_hit_Y < 465))
        {
  ...
  if(b_show_last_hit && (hit_counter > 0))
    {
      ...
      if(projectile_diameter == 0)// for middle evaluation
        ...
        ellipse(last_f_hit_X + video_X_offset, last_f_hit_Y + video_Y_offset, dia, dia);
    };
};

public void write_to_shoot_log()
{
  String current_shoot_log, s_hit_X_temp, s_hit_Y_temp;
  int i_hit_X, i_hit_Y;
  ...
  if(b_Hit_detected && (b_training_style == false))
    ...
    // Add current message
    shoot_logs.add(nf(shoot_log_counter,2) + " " + "X:" + nf(i_hit_X,4) + " Y:" + nf(i_hit_Y,4) + " P:" + nf(hit_points_actual,2) + " T:" +nf(i_time_of_shooting_s,4) );
    // Join logs messages into one string
    txtShootLog.setText(join((String[])shoot_logs.toArray(new String[shoot_logs.size()]), '\n'));
    s_hit_X_temp = (nf(f_hit_X,4,1)).substring(0,4) + "." + (nf(f_hit_X,4,1)).substring(5);
    s_hit_Y_temp = (nf(f_hit_Y,4,1)).substring(0,4) + "." + (nf(f_hit_Y,4,1)).substring(5);
    current_shoot_log =  "X:" + s_hit_X_temp + " Y:" + s_hit_Y_temp + " P:" + String.valueOf(nf(hit_points_actual,2)) + " T:" + String.valueOf(nf(i_time_of_shooting_s,4));
    //s_current_hit_temp
    //current_shoot_log_temp = current_shoot_log;
    
    // 20 lines of header of shootlog file
    s_shoot_logs[shoot_log_counter + 20] = current_shoot_log;  //first six (0~20) lines for shootlog header
      ...
      if(b_autoshootlog && (b_autoshootlog_written == false))
        {
          ...
          System.out.println("Shootlog exported ");
        };
  };
...
void handleImageButtonEvents(GImageButton imagebutton)
{
  ...
  if(b_Configure)
  {
    ...
    if(imagebutton == btnScale_reset)// 
     ...
     if(imagebutton == btnSensitivity_up)// 
     {
       sensitivity_up();
     }; 
     
    if(imagebutton == btnSensitivity_down)// 
     {
       sensitivity_down();
     }; 
...
{
  if(cboTarget_selection == combo)
  {
   s_selected_target_name = cboTarget_selection.selectedText();
